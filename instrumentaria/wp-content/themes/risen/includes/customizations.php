...
if ( ! function_exists( 'risen_logo_url' ) ) {
	...
	function risen_logo_url() {

		// Get uploaded logo
		$logo_url = risen_option( 'logo' );
		...
		if ( empty( $logo_url ) ) {
			...
			$logo_url = risen_locate_template_uri( 'styles/' . $base_style . '/images/logo.png' ); // if image exists in child theme, use it; otherwise use parent theme's file
...
/**
 * Icons Available
 * This is used in displaying icons (risen_icons() in this file) and in Theme Options to tell which social networks are supported (risen_icon_sites())
 */

if ( ! function_exists( 'risen_icon_map' ) ) {
	 
	function risen_icon_map() {

		$icon_map = array( // social media sites with icons (CSS sprites contained within each color scheme)
			'twitter-icon'		=> array( 'twitter',		'Twitter' ),
			'facebook-icon'		=> array( 'facebook',		'Facebook' ),
			'google-plus-icon'	=> array( 'plus.google',	'Google+' ),
			'pinterest-icon'	=> array( 'pinterest',		'Pinterest' ),
			'youtube-icon'		=> array( 'youtube',		'YouTube' ),
			'vimeo-icon'		=> array( 'vimeo', 			'Vimeo' ),
			'flickr-icon'		=> array( 'flickr',			'Flickr' ),
			'stumbleupon-icon'	=> array( 'stumbleupon', 	'StumbleUpon' ),
			'tumblr-icon'		=> array( 'tumblr',			'Tumblr' ),
			'linkedin-icon'		=> array( 'linkedin', 		'LinkedIn' ),
			'skype-icon'		=> array( 'skype', 			'Skype' ),
			'dribbble-icon'		=> array( 'dribbble',		'Dribbble' ),
			'forrst-icon'		=> array( 'forrst',			'Forrst' ),
			'rss-icon'			=> array( array( 'rss', 'feed', 'atom' ), 'RSS' ) // icon CSS class => URL matching string, Site Name
		);
 ...
 * Shown on Theme Options
 */
 
if ( ! function_exists( 'risen_icon_sites' ) ) {
	 
	function risen_icon_sites() {
...
/**
 * Show Icons (Header or Footer)
 */

if ( ! function_exists( 'risen_icons' ) ) {

	function risen_icons( $location = false, $classes = false ) {
		...
		$icon_urls = risen_option( $option_key );

		// Social media URLs defined in options
		if ( ! empty( $icon_urls ) ) {
			...
			foreach ( $url_array as $url ) {
				...
				// URL is valid
				//if ( ! empty( $url ) && ( '[ct_feed_url]' == $url || preg_match( '/^(http(s*)):\/\/(.+)\.(.+)/i', $url ) ) ) { // basic URL check
				if ( ! empty( $url ) && ( '[ct_feed_url]' == $url || preg_match( '/^(http(s*)):\/\/(.+)\.(.+)|skype:(.+)/i', $url ) ) ) { // basic URL check
					...
					foreach ( $icon_map as $icon_class => $site_data ) {
						...
						foreach ( $url_checks as $url_match ) {
							if ( preg_match( '/' . preg_quote( $url_match ) . '/i', $url ) ) {
								$url_matched = true;
								$icon_items .= '	<li><a href="' . esc_attr( $url ) . '" class="single-icon ' . $icon_class . ( $classes ? ' ' . $classes : '' ). '" target="_blank">' . $site_data[1] . '</a></li>' . "\n";				
							}
						}
			...
			if ( ! empty( $icon_items ) ) {
				$icon_list = '<ul id="' . $location . '-icons" class="risen-icon-list' . ( 'header' == $location ? ' light' : '' ) . '">' . "\n";
