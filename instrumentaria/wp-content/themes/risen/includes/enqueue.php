...
function risen_css() {
	...
	// MediaElement.js Stylesheet
	// (before main so can override styles)
	if ( is_singular( 'risen_multimedia' ) ) { // for single Multimedia post pages
		wp_enqueue_style( 'mediaelement', risen_locate_template_uri( 'mediaelement/mediaelementplayer.css' ), false, RISEN_VERSION );  // bust cache on theme update
	}
	...
	if ( ! empty( $font_array ) ) {
		...
		wp_enqueue_style( 'google-fonts', 'http://fonts.googleapis.com/css?family=' . $font_list . $subset_attr, false, null ); // null - don't mess with Google Web Fonts URL by adding version
...
function risen_js() {
	...
	// make responsive features work for IE8
	wp_enqueue_script( 'css3-mediaqueries', risen_locate_template_uri( 'js/css3-mediaqueries.js' ), false, RISEN_VERSION ); // bust cache on theme update
	...
	// Superfish Menu
	wp_enqueue_script( 'hoverintent', risen_locate_template_uri( 'js/hoverIntent.js' ), false, RISEN_VERSION ); // bust cache on theme update
	wp_enqueue_script( 'superfish', risen_locate_template_uri( 'js/superfish.js' ), false, RISEN_VERSION ); // bust cache on theme update
	...
	if ( $gmaps_api_key = risen_option( 'gmaps_api_key' ) ) { // doing it this way w/sensor first helps issue w/old Android issue one user had
	...
	// MediaElement.js
	if ( is_singular( 'risen_multimedia' ) ) { // for single Multimedia post pages
		wp_enqueue_script( 'mediaelement', risen_locate_template_uri( 'mediaelement/mediaelement-and-player.min.js' ), false, RISEN_VERSION ); // bust cache on theme update
	}
